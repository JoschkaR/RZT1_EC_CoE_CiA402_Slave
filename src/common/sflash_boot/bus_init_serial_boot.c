...
void bsc_init(void)
{
    ...
    MSTP(CKIO) = 0;
    MSTP(BSC) = 0;
    dummy = MSTP(BSC);              // Dummy-read for the module-stop state(1)
    dummy = BSC.CS0BCR.LONG;        // Dummy-read for the module-stop state(2)
...
/* If using SPIBSC as default settings, please define "__SPIBSC_DEFAULT__" */
    ...
    /* ==== Sets the SPIBSC ==== */
    sflash_set_config(&g_spibsc_cfg);
    if ( 0 != sflash_exmode_setting(&g_spibsc_cfg) )
    {
        while (1)
        {
